...
$('#link-to-signup').addEventListener('click', (e) => {
  ...
  $('#auth-signup').style.display = 'block';
});
...
async function loadEntries() {
  ...
  try {
    ...
    loadingState.innerHTML = `
      <p style="color:var(--danger);font-weight:500;">Failed to load entries</p>
      <p style="font-size:0.85rem;margin-top:8px;color:var(--text-tertiary);">${escapeHtml(err.message)}</p>
    `;
  }
}
